...
if exist "%PSPATH%" (
   ...
   if exist LL_OSPH_Survey.ps1 (
      rem ll_osph.csv will be created by LL_OSPH_Survey.ps1 in the same directory this batch file is run from
      "%PSPATH%" -ExecutionPolicy Bypass -NoProfile -NonInteractive -File LL_OSPH_Survey.ps1
   )
)
