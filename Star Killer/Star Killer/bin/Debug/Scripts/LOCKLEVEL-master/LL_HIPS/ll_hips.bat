...
if exist "%PSPATH%" (
   ...
   if exist LL_HIPS_Survey.ps1 (
      rem ll_hbss.txt will be created by LL_HIPS_Survey.ps1 in the same directory this batch file is run from
      "%PSPATH%" -ExecutionPolicy Bypass -NoProfile -NonInteractive -File LL_HIPS_Survey.ps1
   )
)
